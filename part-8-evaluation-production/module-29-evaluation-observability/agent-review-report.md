...
**TIER 3 finding**: Section 27.1, LLM-as-Judge biases list could benefit from a brief note on how to combine multiple mitigation strategies in practice.
...
**TIER 3 finding**: Section 27.2 to 25.3 transition could be slightly smoother. The concepts shift from general statistics to RAG-specific evaluation without an explicit bridging sentence in the content.
...
**TIER 2 findings**:
1. Section 27.1: "bits-per-byte" could use a one-sentence plain-language summary after the formal definition
2. Section 27.2: The McNemar's test code uses `&` for array boolean operations without noting this differs from Python's `and`
3. Section 27.3: The RAGAS code example imports from `ragas` without noting the install command
4. Section 27.5: The `@observe()` decorator pattern may be unfamiliar to students who have not used decorator patterns extensively

**TIER 3 findings**:
5. Section 27.6: `deque(maxlen=100)` is used without explaining the `deque` data structure
6. Section 27.7: Hydra's `version_base=None` is not explained
...
**TIER 3 findings**:
- Section 27.1 introduces many metrics (perplexity, BPB, BLEU, ROUGE, BERTScore, METEOR, ChrF, LLM-as-Judge, human eval, Cohen's kappa, MMLU, HumanEval, MT-Bench, Chatbot Arena, GSM8K, SWE-Bench). The metrics taxonomy diagram helps, but this is dense.
- Section 27.3 covers both RAG evaluation AND agent evaluation; each could be its own section.
...
**TIER 2 findings**:
1. Section 27.1, `compute_perplexity_and_bpb`: The BPB calculation line `total_nll_bits = neg_log_likelihood.item() * num_tokens / torch.log(torch.tensor(2.0))` could use an inline comment explaining the nat-to-bit conversion factor
2. Section 27.3: The RAGAS code uses the older API pattern with `from ragas.metrics import faithfulness`; the newer Ragas v0.2+ API uses a different import pattern. Add a version note.
...
**TIER 2 finding**: Section 27.5, platform comparison table says LangSmith is "No" for self-hosting with "(cloud only)". LangSmith actually offers a self-hosted option for enterprise customers. Should say "Enterprise only" (not "No").
...
**TIER 2 findings**:
1. Section 27.1 mentions "instruction-tuned or chat models" without referencing Module 12 (Fine-tuning) or Module 10 (Prompt Engineering)
2. Section 27.3 discusses RAG evaluation but does not reference Module 19 explicitly in the body text
3. Section 27.3 discusses agent evaluation but does not reference Module 21 explicitly in the body text
4. Section 27.7 mentions Docker without referencing any deployment module
...
**TIER 1 finding**: Section 27.5, the LangSmith platform comparison table states LangSmith cannot self-host. This is factually outdated; update to "Enterprise only".
...
**TIER 2 findings**:
1. Section 27.5 should mention that LangSmith now supports self-hosting for enterprise
