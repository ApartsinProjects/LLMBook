...
**Date**: 2026-03-26
**Files Reviewed**: index.html, section-18.1.html, section-18.2.html, section-18.3.html, section-18.4.html
...
**Issues**:
1. **Section 18.2, IPO**: Only gets one paragraph. Missing: how the squared loss specifically prevents overfitting. TIER 2.
2. **Section 18.2, SimPO**: Brief treatment. Could benefit from the actual loss formulation or at least a more concrete description of the length normalization. TIER 3.
3. **Section 18.1, PPO clipping**: The clipping mechanism in PPO is mentioned in code but not explained in prose. TIER 2.
...
**Issues**:
1. **Missing transition between 16.1 and 16.2**: Section 18.1 ends with takeaways but no explicit bridge to why DPO was invented. TIER 3.
2. **Section 18.4**: GRPO is introduced in 16.1 then referenced again in 16.4 without a clear recall bridge. Add "Recall from Section 18.1 that GRPO..." TIER 2.
...
**Issues**:
1. **Section 18.1, PRM code**: The ProcessRewardModel class is a more complex code example; a student might struggle with `step_positions` concept. Add a brief note explaining what step positions represent. TIER 2.
2. **Section 18.2, DPO derivation**: The math from RLHF objective to DPO loss involves 3 steps; intermediate intuitive explanations exist but the partition function cancellation step could use one more sentence. TIER 3.
3. **Section 18.3**: The code for `critique_and_revise` uses `model.generate()` which is pseudocode (not real API). Should note this is simplified/pseudocode. TIER 2.
...
**Issues**:
1. **Section 18.1**: Covers SFT, reward model, PPO, PRM vs ORM, GRPO, and infrastructure in one section. This is 6 major topics. Consider adding more breathing room between subsections. TIER 3.
2. **Section 18.2**: Introduces DPO, KTO, ORPO, SimPO, and IPO. Five methods in one section, but the comparison table helps manage load. TIER 3.
...
**Issues**:
1. **Section 18.1, reward hacking**: Concept is warned about but no concrete example of what a reward-hacked output looks like. TIER 2.
2. **Section 18.3**: The lock-picking analogy for shallow alignment is excellent. Preserve.
3. **Section 18.4**: The student-checking-answer-key analogy for GRPO+RLVR is effective. Preserve.
...
**Issues**:
1. **Section 18.1, PPO code**: Uses `ppo_trainer.dataloader` which is not a standard TRL attribute. Should use a proper data loading pattern. TIER 1.
2. **Section 18.3, Phase 1 code**: `model.generate(tokenizer.encode(critique_input))` is pseudocode; real API needs `return_tensors="pt"` and decode step. Should add a comment marking this as pseudocode. TIER 2.
3. **Section 18.4, code_reward**: Uses `subprocess.run` to execute arbitrary code, which is correct but should include a security warning comment. TIER 3.
4. **Section 18.2, DPO code**: Uses `tokenizer` parameter in DPOTrainer which is deprecated in newer TRL versions (should use `processing_class`). TIER 2.
5. **Section 18.1, SFT code**: Similarly uses `tokenizer` parameter. TIER 2.
...
**Issues**:
1. **Section 18.1, PRM diagram**: The PRM side uses `#fce4ec` (pink) background while steps describe correct process. Color choice slightly misleading (pink often implies error). TIER 3.
...
**Issues**:
1. **Section 18.1**: "OpenAI's InstructGPT used roughly 33,000 human comparisons" is slightly imprecise. The paper reports 33K comparisons for the RM, plus additional prompts for SFT. Acceptable simplification. TIER 3.
2. **Section 18.4, MATH scores**: DeepSeek-R1 at 97.3% on MATH is correct per the paper. QwQ-32B at 90.6% should be verified against latest benchmarks as it may have been updated. TIER 3.
3. **Section 18.2**: "Rafailov et al., 2023" is correct for DPO. TIER 3 (no issue, just verified).
4. **Section 18.2**: IPO attributed to "Azar et al., 2024" is correct. Verified.
5. **Section 18.4**: Sky-T1 "$500 of compute" claim should be qualified as approximate. TIER 3.
...
2. **Section 18.3**: Uses both "AI feedback" and "AI-generated feedback" and "RLAIF." Could standardize. TIER 3.
...
**Issues**:
1. **Section 18.4, GRPO recall**: References "GRPO, introduced in Section 18.1" but should use the actual cross-reference link. TIER 2.
2. **Section 18.3, shallow alignment**: Mentions "Module 17" (interpretability) but does not include a hyperlink. TIER 2.
3. **Section 18.2**: References "Section 18.3" for RLAIF; this is correct and well-placed.
...
**Issues**:
1. **Between 16.2 and 16.3**: The transition is implicit. Section 18.2 ends with takeaways about DPO variants; 16.3 opens with the annotation bottleneck. A bridge sentence at the end of 16.2 pointing toward CAI's motivation would help. TIER 3.
...
**Issues**:
1. **Section 18.1**: The section is information-dense. Could benefit from a "Did You Know?" callout about the cost of RLHF annotation at OpenAI. TIER 3.
2. **Section 18.4**: The "aha moment" narrative about DeepSeek-R1 is inherently engaging. Preserve.
...
3. **Section 18.2**: The connection between DPO and contrastive learning could be noted as a research insight. TIER 3.
...
2. **Section 18.2**: After the DPO code example, a callout like "You Could Build This: Fine-tune a small model on preference data using DPO and evaluate it against the SFT baseline" would add action orientation. TIER 3.
3. **Section 18.4**: After the RLVR code, a callout about replicating basic GRPO on a small math dataset would be valuable. TIER 3.
...
- "RLHF is the technique that turned GPT-3 into ChatGPT" (Section 18.1)
- "DPO achieves RLHF-level alignment without reinforcement learning" (Section 18.2)
- The DeepSeek-R1 "aha moment" narrative (Section 18.4)
- The lock-picking analogy for shallow alignment (Section 18.3)

**Issues**:
1. **Section 18.2**: The partition function cancellation is mathematically elegant but not framed as an aha moment. Adding "This is the key trick" emphasis would help. TIER 3.
...
**Issues**:
1. **Section 18.1 PRM diagram**: Uses `#fce4ec` (pink) on PRM side which is slightly different from the standard palette. TIER 3.
...
- Section 18.4 is essentially a research frontier section (RLVR is cutting-edge)
...
**Issues**:
1. **Section 18.2**: A parameter sensitivity demo showing how beta affects DPO training (plot of reward margin vs beta) would be valuable. TIER 3.
2. **Section 18.1**: A "Run This Now" moment showing reward model scoring of two responses would be compelling. TIER 3.
...
2. **Section 18.1**: The four models needed for PPO (policy, reference, reward, value) could use a mnemonic or memorable framing. TIER 3.
...
**Issues**:
1. **Section 18.1, SFT stage**: The SFT explanation is somewhat standard/generic. Most RLHF resources cover this similarly. TIER 3 (acceptable since it is foundational).
2. **Section 18.2**: The DPO derivation walkthrough is well-done and above the level of a typical blog post.
...
**Issues**:
1. **Section 18.1**: "The alignment problem is the challenge of bridging this gap: how do we take a capable base model and steer its behavior to match human intentions?" could start with the direct question. TIER 3.
...
**Issues**:
1. **"KL divergence"**: Used in Section 18.1 without a brief definition. Important jargon that needs inline explanation on first use. TIER 1.
2. **"Bradley-Terry"**: Explained well with formula and intuition. Good.
3. **"RLAIF"**: Expanded on first use in Section 18.3. Good.
4. **"Prospect theory"**: Mentioned in KTO description (Section 18.2) without explanation. TIER 2.
...
**Issues**:
1. **Section 18.1**: The infrastructure section (Section 6) is somewhat brief for such an important topic. Could be slightly expanded. TIER 3.
...
**Energy Map**:
- Section 18.1: HIGH (strong hook, good progression, excellent diagrams)
- Section 18.2: HIGH (clear derivation, practical code, good comparison table)
- Section 18.3: MEDIUM-HIGH (good CAI explanation, slight energy dip in Phase 2 code)
- Section 18.4: HIGH (exciting topic, emergent reasoning narrative is compelling)

**Issues**:
1. **Section 18.3, middle**: The RLAIF code block is long and could benefit from a brief inline comment explaining the flow before the code. TIER 3.
...
1. **KL divergence definition**: Add inline definition on first use in Section 18.1
2. **PPO code pattern**: Fix `ppo_trainer.dataloader` to use proper TRL pattern

### TIER 2 (High Priority): 10 fixes
3. **Section 18.4 GRPO recall**: Add "Recall from Section 18.1" bridge
4. **Section 18.3 shallow alignment Module 17 link**: Add hyperlink
5. **Section 18.1 PRM code**: Add explanatory note about step_positions
6. **Section 18.3 code pseudocode note**: Mark critique_and_revise as simplified/pseudocode
...
9. **Section 18.1 PPO clipping**: Add brief prose explanation of clipping mechanism
...
18. Section 18.2 to 16.3 transition bridge
...
36. Section 18.3 code pseudocode marker for build_cai_sft_dataset
