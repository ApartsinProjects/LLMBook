...
**Date**: 2026-03-26
**Files reviewed**: index.html, section-28.1 through section-28.11 (13 files)
...
- **Issue 1**: Section 28.6 NLI code uses `pipeline("text-classification")` with `candidate_labels`, but the `text-classification` pipeline does not accept `candidate_labels` (that is `zero-shot-classification`). TIER 1 fix.
- **Issue 2**: Section 28.3 mentions "continuous batching (vLLM)" in a diagram label but never explains what continuous batching is or why it matters. Add a brief explanatory sentence. TIER 2 fix.
- **Issue 3**: Section 28.11 mentions LOKA in the subtitle and diagram but the prose explanation is only in a Note callout. Deserves a brief paragraph in the main text. TIER 3 fix.
...
- **Issue 1**: Section 28.5 quiz answer 3 uses "trivially" ("Attackers can trivially evade regex"). Replace with "easily." TIER 1 fix.
- **Issue 2**: Section 28.8 uses "simply" ("you cannot simply delete"). Replace. TIER 1 fix.
- **Issue 3**: Section 28.11 uses "simply" ("A model that simply refuses"). This usage is acceptable as it contrasts genuine unlearning with surface-level refusal.
...
- **Issue**: Section 28.3 guardrails table introduces 6 tools in rapid succession. The prose after the table helps, but a brief introductory sentence before the table framing the landscape would reduce load. TIER 3 fix.
...
- **Issue 1**: Section 28.6 NLI code has an API mismatch (text-classification vs zero-shot-classification). TIER 1 fix.
- **Issue 2**: Section 28.4 PromptRegistry `get` method returns latest by sort order of glob, which is alphabetical, not chronological. The `created_at` timestamp could be used instead. Minor pedagogical note. TIER 3 fix.
...
- **Issue 1**: Section 28.6 states "RAG reduces but does not eliminate hallucination." Good. This is a common misconception that is correctly addressed.
- **Issue 2**: Section 28.11 correctly distinguishes output suppression from true unlearning. Important misconception addressed well.
- **No additional misconceptions left unaddressed.**

### Agent 11: Fact Integrity
- **Issue 1**: Section 28.6 code uses `pipeline("text-classification", model="facebook/bart-large-mnli")` and then calls it with `candidate_labels`. The `text-classification` pipeline does not support `candidate_labels`; should be `zero-shot-classification`. TIER 1 fix.
- **Issue 2**: Section 28.7 states "datasheets for datasets by Gebru et al. (2021)." The original datasheets paper was published in 2018 (arXiv) and 2021 (CACM). The year is acceptable if referring to the CACM version, but could be clarified. TIER 3 fix.
- **Issue 3**: Section 28.10 Llama license table says "Llama 3, Llama 3.1" but the Llama Community License also covers Llama 3.2 and 3.3. Update for completeness. TIER 2 fix.
...
- **Issue 2**: Only one internal cross-reference exists (26.8 references Section 28.11 for machine unlearning). Need more internal cross-linking between sections. TIER 2 fix.
...
- **Issue 1**: Section 28.11 does not mention the NeurIPS 2023 Machine Unlearning Challenge or the TOFU benchmark, which are important recent developments. TIER 3 fix.
- **Issue 2**: Section 28.5 does not mention adversarial suffix attacks (Zou et al. 2023) or the universal jailbreak research. TIER 3 fix.
...
- **Issue 1**: Section 28.10 Llama license table should include Llama 3.2/3.3. TIER 2 fix.
- **Issue 2**: Section 28.3 could mention Llama Guard 4's multimodal capabilities more prominently, as it was released in 2025. Already mentioned briefly.
...
- **Strength**: Section 28.5's sandwich defense code is a good aha moment (seeing the defense in action).
- **Issue**: Section 28.11's task vector concept could benefit from a more visceral demonstration showing before/after behavior. TIER 3 fix.
...
- **Issue 1**: Section 28.3 Big Picture: "Scaling involves more than adding replicas; it requires latency optimization at every layer (caching, batching, model quantization), backpressure mechanisms to prevent cascading failures, and guardrails that inspect both inputs and outputs in real time." This 40+ word sentence could be split. TIER 3 fix.
...
- **Issue 1**: Section 28.3 uses "backpressure" without definition before explaining it. The term appears in the Big Picture but is only defined in Section 2's heading. Add a brief inline definition on first use. TIER 2 fix.
- **Issue 2**: Section 28.9 uses "three lines of defense" without explaining it is a banking/risk management framework concept. TIER 2 fix.
- **Issue 3**: Section 28.4 uses "data flywheel" in the Big Picture without defining it until later. TIER 3 fix.

### Agent 34: Micro-Chunking Editor
- **Sections are well-chunked**. Each section has 2-4 numbered subsections with clear scope.
- **Issue**: Section 28.8 covers EU AI Act, GDPR, and sector-specific regulations all in one section. Each is a substantial topic. Consider whether it should be split. Current treatment is adequate given the overview nature. TIER 3.
