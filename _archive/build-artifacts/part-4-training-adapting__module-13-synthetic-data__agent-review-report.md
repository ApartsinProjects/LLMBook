...
Date: 2026-03-26
Reviewed files: index.html, section-14.1.html through section-14.6.html
...
### Issues
1. MEDIUM: Section 14.1, "Data Contamination" (line ~491): explains what contamination is but does not give a concrete detection method. A brief mention of n-gram overlap checks or benchmark contamination testing would complete the picture.
2. MEDIUM: Section 14.4, semantic dedup: the code uses a naive O(n^2) approach but does not mention that production systems use approximate nearest neighbor (FAISS, Annoy) for scalability. The diagram mentions FAISS but the code does not address this.
3. LOW: Section 14.6, the Snorkel label model explanation says it "learns a generative model" but does not briefly explain what that means (it models P(LF votes | true label) and infers P(true label | LF votes) via EM).
...
2. LOW: Section 14.3 (simulation) could optionally come after 12.4 (QA) since you want to curate before using synthetic data. However, the current order works because simulation is a different use case (testing, not training).
...
### Issues
1. HIGH: Section 14.4, the MinHash dedup code imports `datasketch` without noting it is a third-party library requiring installation. A student running this code would get an ImportError. Add a note: "Install with `pip install datasketch`."
2. MEDIUM: Section 14.5, the `find_optimal_threshold` function uses simulated data, which is good for illustration but the student might wonder how to get `gold_labels` in practice. Add a sentence explaining that you label a small calibration set (100-200 examples) by hand.
3. MEDIUM: Section 14.6, the Snorkel/FlyingSquid comparison diagram mentions FlyingSquid but no code or further explanation is provided. Either add a brief code snippet or a link.
4. LOW: Section 14.3, the red-teaming code uses `response_format={"type": "json_object"}` but also asks for a JSON array in the prompt. The `json_object` mode requires the top-level to be an object, not an array. The code handles this with `result.get("prompts", ...)` but this mismatch could confuse students. Add a note.
...
### Issues
1. HIGH: Section 14.4, `minhash_dedup` imports `datasketch` mid-function. Best practice is to show imports at the top of the code block or add a comment noting the dependency.
2. MEDIUM: Section 14.5, `LLMPreLabeler` class inherits from `LabelStudioMLBase` but the import is from `label_studio_ml.model`, which requires the `label-studio-ml` package. Add a comment noting this dependency.
3. MEDIUM: Section 14.4, semantic_dedup code uses `client.embeddings.create` but `client` is not defined in that code block. It relies on the earlier definition in the same section. This could confuse a reader who runs the code in isolation. Add `client = OpenAI()` to the block or note the dependency.
...
### Visual Inventory:
- Section 14.1: 3 SVG diagrams (Four Drivers, Quality Dimensions, Lifecycle)
- Section 14.2: 3 SVG diagrams (Self-Instruct pipeline, Conversation synthesis, Preference data)
- Section 14.3: 2 SVG diagrams (User simulator architecture, Evaluation harness)
- Section 14.4: 3 SVG diagrams (Dedup levels, Filtering pipeline, Quality scoring table)
- Section 14.5: 2 SVG diagrams (Pre-labeling workflow, Active learning loop)
- Section 14.6: 2 SVG diagrams (Snorkel paradigm, Aggregation approaches)
...
### Issues
1. NEEDS QUALIFICATION: Section 14.1, "GPT-4o synthetic generation: $0.005-$0.02 per example." Pricing changes frequently. Add "(as of early 2025; check current pricing)" caveat.
2. NEEDS QUALIFICATION: Section 14.2, "Studies on the Orca and Phi datasets showed that persona-driven generation improved downstream model performance by 5% to 15%." This should specify the benchmark or metric. The claim is directionally correct but vague.
3. POTENTIALLY OUTDATED: Section 14.4, Distilabel code uses `TextGeneration` and `UltraFeedback` step names. Distilabel's API has evolved; verify these match the current stable release.
...
2. MEDIUM: No forward references to Module 13 (Fine-tuning). Section 14.6 ends without previewing how the labeled data will be used. The nav link exists but prose is missing.
3. LOW: Internal cross-references between sections are absent. Section 14.4 discusses LLM-as-judge scoring but does not reference 12.3's evaluation harness discussion, which covers the same concept from a different angle.
...
3. MEDIUM: Section 14.3 could include: "You Could Build This: A synthetic user testing suite for any chatbot, complete with persona library and automated evaluation."
...
### Issues
1. HIGH: Section 14.6 needs an aha moment showing that combining weak LFs with an LLM outperforms either alone. The code shows this (82.4% majority vote vs 85.0% weighted) but the difference is modest. A more dramatic example with a clearer gap would strengthen the case.
2. MEDIUM: Section 14.4, dedup code shows removal percentages but does not show a before/after example of what a near-duplicate looks like. Adding a concrete pair of near-duplicate instructions would make the concept click.
...
### Issues
1. MEDIUM: Section 14.1, the diversity measurement code would be more impactful as a "Run This Now" moment with the student generating their own texts and measuring diversity.
2. MEDIUM: Section 14.5, the confidence threshold finder would be an excellent slider/parameter exploration demo: "Try changing `budget_fraction` from 0.1 to 0.5 and observe how the optimal threshold shifts."
3. LOW: Section 14.2, Evol-Instruct chain could encourage students to try different seed instructions.
...
### Issues
1. LOW: Section 14.6, line ~477: "LLM-generated labels are simply another labeling function in the weak supervision framework, but a particularly powerful one." This is clear. No changes needed.
...
### Issues
1. LOW: Section 14.1, the legal/ethical table (lines ~499-506) is followed by a note callout and then the lifecycle section. The transition is slightly abrupt. Add a bridge sentence.
...
### Issues
1. MEDIUM: Section 14.4 is the lowest-energy section. The filtering pipeline is important but procedural. Add a motivating example at the start showing what happens when you train on uncurated data (specific failure case).
