...
Consult the book's `CONFORMANCE_CHECKLIST.md`, `SKILL.md`, or table of contents (`index.html`) for the current list of parts, modules, and their folder paths. The module map is book-specific and should not be hardcoded in this agent definition.

When running, read the book's top-level `index.html` or the project's configuration files to discover the module structure dynamically.
