...
```
E:/Projects/LLMCourse/
├── index.html                          # Book-level ToC (lists all 7 parts + appendices)
├── part-1-foundations/
...
│   ├── index.html
│   └── module-26-production-engineering/
│       ├── index.html
│       └── ...
└── appendices/
...
1. **Book index** (`index.html`): Lists all 7 parts with correct names and links. Lists all appendices (A through J). Module count per part is accurate.
...
- After Module 26 split: verify Part 7 now shows modules 26, 27, 28 (not old structure)
