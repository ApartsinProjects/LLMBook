...
def _is_inside_code_block(html, pos):
    """Check if position is inside <pre>, <code>, <script>, or code-caption."""
    before = html[:pos]
...
def _is_inside_math_container(html, pos):
    ...
    # Check for span.math
    last_span_open = before.rfind('<span class="math">')
    last_span_close = before.rfind('</span>')
    if last_span_open > -1 and last_span_open > last_span_close:
        return True

    # Check for div.math-block
    last_div_open = before.rfind('<div class="math-block">')
    last_div_close = before.rfind('</div>')
    if last_div_open > -1 and last_div_open > last_div_close:
        return True

    # Check for single $ (inline math) -- count $ signs not part of $$
    # This is approximate; we strip $$ first
...
def check_orphaned_math(html):
    ...
    for m in LATEX_LIKE_RE.finditer(html):
        ...
        if _is_inside_math_container(html, pos):
            continue
