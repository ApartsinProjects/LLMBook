...
| # | File | Fix |
|---|------|-----|
| 1 | `part-15-llm-agentic-ai-research-frontiers/module-75-frontier-architectures/section-75.3.html` (80.3.4.2 Design Principles for Hybrid Architectures) | Converted three bolded "Attention layer placement / The ratio depends on the task / Sliding-window attention" paragraphs into a `<ul>` with a 1-sentence bridge ("...three design principles, each answering a different 'where do I put the attention?' question"). |
| 2 | `part-15-llm-agentic-ai-research-frontiers/module-75-frontier-architectures/section-75.3.html` (80.3.6 When to Consider Non-Transformer Architectures) | Promoted three bolded scenarios ("Extremely long contexts / High-throughput streaming / Edge and mobile") into `<h3>` sub-subheadings (80.3.6.1, .2, .3) with a 1-sentence bridge ("three scenarios justify the switch, each driven by a different hardware or workload constraint"). |
| 3 | `part-14-applications-of-llms-across-industries/module-73-manufacturing-llms/section-73.9.html` (78.9.5.4 Augmented Analytics Platforms) | Converted the three vendor sentences (Tableau AI / ThoughtSpot Sage / Power BI Copilot) buried in prose into a `<ul>` with a 1-sentence bridge ("Three flagship platforms illustrate how the same idea (LLM on top of a BI engine) gets realized differently"). |
| 4 | `part-14-applications-of-llms-across-industries/module-73-manufacturing-llms/section-73.9.html` (78.9.5.5 Challenges in NL-to-Analytics) | Converted four bolded inline challenges ("Hallucinated SQL / Schema misinterpretation / Data confidentiality / Trust in automated insights") into a `<ul>` with a 1-sentence bridge ("ordered from most to least dangerous in production"). |
| 5 | `part-15-llm-agentic-ai-research-frontiers/module-76-frontier-theory/section-76.2.html` (81.2.3 Working Memory vs. Long-Term Memory) | Converted two bolded paragraphs ("Working memory / Long-term memory") into a `<ul>` with a 1-sentence bridge ("Each maps to a different storage substrate with different capacity and latency characteristics"). |
| 6 | `part-14-applications-of-llms-across-industries/module-69-healthcare-llms/section-69.4.html` (Choosing Among the Four Patterns) | Converted the four-pattern enumeration in a single 215-word paragraph into a `<ul>` with a 1-sentence bridge ("Each of the four patterns lands on a different answer to that pair"). |
| 7 | `part-14-applications-of-llms-across-industries/module-69-healthcare-llms/section-69.4.html` (Cross-Pattern Considerations) | Converted the three-consideration inline list ("First, the BAA must cover... Second, training and fine-tuning... Third, audit logs...") into a proper `<ol>` with a 1-sentence bridge ("Three considerations cut across all four patterns, each of which has tripped a real institution at least once during procurement"). |
| 8 | `part-14-applications-of-llms-across-industries/module-73-manufacturing-llms/section-73.4.html` (Reference Stack and Vendor Choices) | Broke the 357-word "everything-on-one-line" reference-stack paragraph into a six-item `<ul>` (Inference / Embedding models / Vector stores / Orchestration / Observability / MES and CMMS handoff) with a 1-sentence bridge ("Six components show up in almost every deployment we have seen"). |
| 9 | `part-14-applications-of-llms-across-industries/module-68-finance-llms/section-68.4.html` (Tier 0: Decisions That Cannot Be LLM-Mediated) | Converted the four Tier-0 workflows from a single dense paragraph into a `<ul>` with a 1-sentence bridge ("Four workflows account for almost every Tier 0 conversation at a U.S. bank"). |
| 10 | `part-14-applications-of-llms-across-industries/module-68-finance-llms/section-68.4.html` (Tier 3: Customer-Facing With Guardrails) | Converted the five inline-numbered guardrails into a proper `<ol>` with a 1-sentence bridge ("five interlocking controls, each of which has been breached at least once in an industry pilot when shipped alone"). |
| 11 | `part-14-applications-of-llms-across-industries/module-69-healthcare-llms/section-69.2.html` (Confident Wrong Answers in High-Stakes Contexts) | Converted four inline-numbered mitigations into a proper `<ol>` with a 1-sentence bridge ("four layers stacked, each of which has failed alone in at least one published incident"). |
| 12 | `part-14-applications-of-llms-across-industries/module-69-healthcare-llms/section-69.2.html` (Privacy Leakage) | Converted four inline-numbered mitigations into a proper `<ol>` with a 1-sentence bridge ("The mitigation pattern is four layers; getting any one wrong reintroduces the leakage"). |
| 13 | `part-14-applications-of-llms-across-industries/module-71-cybersecurity-llms/section-71.3.html` (Prompt Injection) | Converted three inline-numbered defences (input classification / output filtering / scoped tool permissions) into a proper `<ol>` with a 1-sentence bridge ("three layers, none of which is sufficient on its own"). |
| 14 | `part-14-applications-of-llms-across-industries/module-71-cybersecurity-llms/section-71.3.html` (Membership-Inference and Extraction) | Converted three inline defences (differential privacy / output filtering / rate-limiting) into a `<ul>` with a 1-sentence bridge ("Three defenses stack"). |
| 15 | `part-14-applications-of-llms-across-industries/module-71-cybersecurity-llms/section-71.3.html` (Model Extraction and Stealing) | Converted three inline enforcement mechanisms (rate limits / watermarking / ToS provisions) into a `<ul>` with a 1-sentence bridge ("Major model providers have moved to active enforcement on three fronts"). |
| 16 | `part-14-applications-of-llms-across-industries/module-73-manufacturing-llms/section-73.7.html` (Suno and Udio) | Broke the 190-word combined paragraph by extracting the four operational disciplines into a `<ul>` with a 1-sentence bridge ("four operational disciplines have stabilized regardless of how the litigation resolves"). |
| 17 | `part-14-applications-of-llms-across-industries/module-73-manufacturing-llms/section-73.6.html` (Warning callout, Legal landscape) | Converted three inline-numbered IP issues (training data licensing / authorship / right of publicity) into a proper `<ol>` inside the existing Warning callout. |
...
- `part-15-llm-agentic-ai-research-frontiers/module-76-frontier-theory/section-76.3.html` (Mechanistic Interpretability): already uses `<ol>`/`<ul>` for the circuit-discovery pipeline, `<h3>` sub-headings for each practical application, and short focused paragraphs throughout. No structural change needed.
- `part-15-llm-agentic-ai-research-frontiers/module-76-frontier-theory/section-76.4.html` (The Nature of Agency): the five-level agency ladder is already a structured set of `<h3>` sub-sections with one paragraph each; the four-dimension agency decomposition is already a `<ul>`. Well-paced.
- `part-14-applications-of-llms-across-industries/module-71-cybersecurity-llms/section-71.4.html` (Five-Layer Trust-Boundary Pattern): already an `<ol>` of layers followed by per-layer `<h2>` sub-sections. Well-paced.
- `part-14-applications-of-llms-across-industries/module-70-education-llms/section-70.4.html` (Pedagogically-Scaffolded Tutor Architecture): five layers as `<ol>` plus per-layer `<h2>` sub-sections, parallel to 76.4. Well-paced.
- `part-14-applications-of-llms-across-industries/module-72-government-llms/section-72.4.html` (Seven-Layer Public-Sector Pattern): seven layers as `<ol>` plus a "Layer Notes" section with seven short paragraphs. Well-paced.
- `part-14-applications-of-llms-across-industries/module-73-manufacturing-llms/section-73.1.html`, `78.2.html`, `78.5.html`, `78.6.html` (mostly): each one uses `<h2>` sub-headings every 1-2 paragraphs and lists for any enumeration of 3+ items. Well-paced apart from the specific fixes listed above.
