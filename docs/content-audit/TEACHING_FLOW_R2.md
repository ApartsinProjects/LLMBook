...
3. **`section-3.1.html`** (Transformer Architecture)
...
4. **`section-3.6.html`** (GPU Architecture; three bridges)
   - Between `3.6.2 GPU Architecture Overview` and `3.6.2.1 Streaming Multiprocessors (SMs)`: 300x compute-to-memory-bandwidth gap motivates the SM vocabulary.
   - Between `3.6.3 Compute-Bound vs. Memory-Bound Operations` and `3.6.3.1 The Roofline Model`: roofline as the diagnostic chart for every kernel.
   - Between `3.6.7 Practical Considerations` and `3.6.7.1 Mixed Precision Training`: workflow-level choices (precision, memory budgeting, debugging) for hitting 50%+ MFU.
...
8. **`section-7.1.html`** (Modern LLM Landscape: Anthropic)
...
9. **`section-7.2.html`** (Modern LLM Landscape; two bridges)
   - Between `7.2.2 Second-Tier Frontier Models` and `xAI Grok`: each lab carves a defensible niche (real-time data, RAG citations, European sovereignty).
   - Between `7.2.3 Comparing the Frontier` and `Capability Dimensions`: no single benchmark answers the "which one to pick" question.
...
11. **`section-9.3.html`** (Long-Context Attention)
    - Between `9.3.9 Research Frontiers` (h2) and `9.3.9.1 Test-Time Training (TTT)` (h3)
    - Frame: from deployable techniques (YaRN, LongRoPE, Ring Attention) to rethinking the cache itself (TTT, DSA).

12. **`section-9.4.html`** (Speculative Decoding)
    - Between `9.4.6 Practical Implementation` (h2) and `From-Scratch Implementation` (h3)
    - Frame: shortest path from theory to running code is a 40-line draft-verify-resample loop.

13. **`section-9.8.html`** (Reasoning Models; two bridges)
    - Between `9.8.2 Reasoning Models: Architecture and Training` and `9.8.2.1 The "Thinking Tokens" Paradigm`: 70-point AIME jump motivates how to teach this behavior.
    - Between `9.8.5 Using Reasoning Models in Practice` and `9.8.5.1 API Usage`: three practical API differences (thinking budget, billing, streaming).
...
18. **`section-17.5.html`** (Distillation; two bridges)
...
19. **`section-17.7.html`** (Model Merging)
    - Between `17.7.2 Merging Methods` (h2) and `17.7.2.1 Linear (Weighted Average)` (h3)
    - Frame: four merging families (linear, SLERP, TIES, DARE), each fixing a failure mode of the last.

20. **`section-18.2.html`** (RLHF/GRPO Practical Tips)
    - Between `18.2.4 Practical Tips for RL-Based Alignment` (h2) and `Learning Rate and Schedule` (h3)
    - Frame: short checklist (small LR, cosine warmup, watch KL) from teams who shipped RLHF/DPO.

21. **`section-18.4.html`** (DPO Hyperparameters)
    - Between `18.4.4 Practical Considerations for DPO Training` (h2) and `18.4.4.1 Hyperparameter Sensitivity` (h3)
...
23. **`section-31.2.html`** (Embeddings; three bridges)
    - Between `31.2.2 Embedding Model Ecosystem and Selection` and `API Embedding Services`: API vs self-hosted as the two camps.
    - Between `31.2.3 Embedding Space Geometry` and `Similarity Metrics`: similarity metric and dimensionality, both swing quality without model change.
    - Between `31.2.5 Practical Considerations` and `Query and Document Prefixes`: three production gotchas (prefixes, asymmetric processing, re-embedding versioning).

24. **`section-31.4.html`** (Vector Indexes)
    - Between `31.4.2 Composite Indexes and Advanced Techniques` (h2) and `IVF-HNSW` (h3)
    - Frame: stacking two approximations (HNSW for traversal + PQ for compression) to exploit different cost-budget axes.

25. **`section-31.5.html`** (Vector Databases; three bridges)
    - Between `31.5.2 Purpose-Built Vector Databases` and `Pinecone`: four competitors on different axes (Pinecone, Qdrant, Weaviate, Milvus).
    - Between `31.5.3 Lightweight and Embedded Solutions` and `ChromaDB`: lighter tier trades replication for zero-infra embedded use.
    - Between `31.5.7 Operational Considerations` and `Scaling Patterns`: scaling, backup, observability as the three operational levers.

26. **`section-31.6.html`** (RAG Ingestion; two bridges)
    - Between `31.6.2 Document Parsing` and `The PDF Challenge`: PDFs deserve special attention because they dominate enterprise corpora and break naive extraction.
    - Between `31.6.4 Overlap and Parent-Child Retrieval` and `Chunk Overlap`: overlap and parent-child retrieval as complementary fixes for chunking boundaries.
...
28. **`section-47.3.html`** (Red Teaming Tools)
    - Between `47.3.2 Automated Red Teaming Tools` (h2) and `47.3.2.1 Microsoft PyRIT` (h3)
