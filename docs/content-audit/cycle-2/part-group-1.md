...
**Chapter 0 (module-00)**: title "Chapter 00" / breadcrumb "Chapter 0" zero-padding mismatch. Body refs: "Chapter 26" (agents, fine), "Chapter 04 Transformer" (real 3), "Chapter 20: Alignment, RLHF & DPO" (real 18), "Chapter 18 Fine-Tuning Fundamentals" (real 16). Figure 0.0.1 caption OK. Section 0.4 (RL Foundations) body has 3 references to "Section 20.1" / "Chapter 20" pointing at section-18.1/module-18 — stale visible text on otherwise-correct hrefs.
...
**Chapter 7 (module-07)**: title "Chapter 07" / breadcrumb "Chapter 7". Figures "Figure 8.X.Y" (off +1). Section 7.3 body: "Chapter 18" (real 16, fine-tuning) appears multiple times. Section 7.2 body: "Chapter 15" (real 13, hybrid). H2 visible text "8.1.1" while IDs use "7-1-1".
...
**Chapter 17 (module-17)**: title and breadcrumb at the chapter index say "Chapter 17: Parameter-Efficient Fine-Tuning, Distillation & Model Merging" (Wave 16 rename). All seven section breadcrumbs (section-17.1 through 17.7) still link to `index.html` with the OLD label `Chapter 17: Parameter-Efficient Fine-Tuning (PEFT)` — Wave 16 stopped at the index. Figures "Figure 19.X.Y" (off +2). Tables "Table 19.X.Y". Section 17.7 What's Next says "Chapter 20: Alignment: RLHF, DPO & Preference Tuning" (real 18).

**Chapter 18 (module-18)**: title and breadcrumb both "Chapter 18". Figures "Figure 20.X.Y" (off +2). H2 visible "20.X.Y", IDs "18-X-Y". Section 18.5 body: "Section 37.12 AI Governance" → target points to module-55 in Part 11 (real Section 55.2 or similar), "Chapter 37" → target points to module-47 (real Chapter 47).

**Chapter 19 (module-19, Tools of the Trade)**: title and breadcrumb both "Chapter 19". H2 visible "21.X.Y" (off +2), IDs "19-X-Y". MAJOR INFLATION: section 19.2 has 10 chapter-nav blocks (nav-num labels "Section 19.5" through "Section 19.13" pretending to be siblings); section 19.3 has 5. Self-pointing hrefs like `section-19.2.html#21-2-...` with old-numbering anchors. Section 19.4 body: "Chapter 34" eval (real 42), "Chapter 23" RAG (real 32). Section 19.5 looks clean.
...
- Part-index chapter cards rebuilt from filesystem: MOSTLY resolved, except (a) Part-2 chapter-6 card has a duplicate section-6.9 entry; (b) Part-2 index does NOT list sections 10.5-10.9 even though they exist physically; (c) Part-1 index does NOT list sections 1.5-1.7 (tokenization).
...
3. **H2 subheading visible-text renumbering.** Where `<h2 id="X-Y-Z">A.B.C ...</h2>` mismatches X-Y-Z != A-B-C, rewrite the visible text to match the ID. Affected modules: Part 1 (2/3/4), Part 2 module-10 sections 10.5-10.9, Part 3 module-14, Part 4 modules 15/16/17/18/19. Single regex pass per file.
