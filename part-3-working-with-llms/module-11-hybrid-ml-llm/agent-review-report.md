...
**Date:** 2026-03-26
**Files reviewed:** index.html, section-11.1.html through section-11.5.html, chapter-plan.md
...
2. Section 11.2 is the shortest section (710 lines) and could benefit from a TF-IDF vs. embeddings baseline comparison.
...
- Data privacy was added to the decision axes in Section 11.1, which the plan identified as missing. Good.
...
**Issues (TIER 3):**
1. Section 11.1 ends with takeaways but no explicit "Now that you know when to use each approach, the next question is how to combine them. Section 11.2 shows the first integration point: using LLM embeddings inside classical pipelines." Adding such a bridge sentence to each section's closing would improve flow.
...
**Issues (TIER 3):**
1. Section 11.4, subsection 3 (Token Optimization) covers compression, semantic caching, and batching in quick succession. Each gets a brief treatment. This is fine as a survey but a "summary of optimization strategies" mini-table after subsection 3.3 would help consolidation.
...
2. Section 11.4 could benefit from a matplotlib-style Pareto frontier scatter plot with the frontier line highlighted, as the chapter plan noted.
...
- Section 11.2 cross-references Module 01 (text representation) and Module 07 (pretraining).
- Section 11.4 cross-references Section 9.3 (semantic caching) with a clear note distinguishing the engineering vs. cost optimization framing.
- Section 11.5 references Section 10.3 (DSPy) in the forward-looking callout.

**Issues (TIER 2):**
1. **11.1:** The LLM bootstrap pattern callout says "We will explore this pattern in depth in Section 11.3." However, 11.3 does not explicitly revisit the bootstrap pattern by name. It covers triage and escalation patterns. Either adjust the forward reference to point to 11.3's triage pattern or add a brief mention of the bootstrap pattern in 11.3.
2. **11.5:** The Instructor usage would benefit from referencing Section 9.2's detailed treatment, as the chapter plan notes.
...
- Section 11.5's closing callout ("What Makes This Module Distinctive") effectively wraps the entire module's narrative.

**Issues (TIER 3):**
1. Section 11.4 does not use the customer support running example. Connecting the TCO analysis back to the customer support pipeline from 11.3 would strengthen the thread.
...
- Section 11.2 includes a prerequisite callout linking to Modules 01 and 07 for embedding fundamentals.
- Section 11.4 cross-references Section 9.3 for semantic caching.
...
**Issues (TIER 3):**
1. Section 11.2's subtitle "Using embeddings and LLM-generated features to supercharge classical ML pipelines" uses "supercharge" which is slightly informal. Consider: "Using embeddings and LLM-generated features to strengthen classical ML pipelines."
...
- Section 11.5 includes a forward-looking "Where This Leads Next" callout mentioning compound AI systems, DSPy, and AI engineering.
- Section 11.5 includes a "What Makes This Module Distinctive" callout.
...
**Issues (TIER 3):**
1. Section 11.5 is the longest and most concept-dense section. The final sections (5. Production Deployment Patterns, 6. Financial Event Extraction) are lighter, which provides good pacing after the dense middle (classical IE, LLM IE, hybrid architecture).
...
| 12 | 11.5 | Add Instructor cross-reference to Section 9.2 | 13 |
