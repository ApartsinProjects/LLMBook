...
**Date:** 2026-03-26
**Files Reviewed:** index.html, section-27.1.html through section-27.5.html
...
**Unjustified Claims:**
1. Section 27.3: "Studies from GitHub and Google report 20 to 55% faster task completion" needs specific citation. (MEDIUM)
2. Section 27.5: GPU TFLOPS numbers stated without source. (LOW, industry specs)
3. Section 27.4: Provider scores (OpenAI 4.8, Anthropic 4.7, etc.) are illustrative but the Note box adequately disclaims this.

**Missing Intuition:**
1. Section 27.5: The breakeven formula is shown in code but not explained intuitively. Why does breakeven = fixed_cost / variable_cost_per_request? A brief "think of it as: how many requests until the saved pennies pay for the GPU" would help. (TIER 2)
2. Section 27.3: ROI formula shown but the "why divide by cost" step could use a one-sentence intuition. (TIER 3)
...
**Missing Transitions:**
1. Between Section 27.1 and 27.2: The index page has descriptions but sections themselves could open with a bridge sentence referencing what was just learned. (TIER 2)
2. Between Section 27.4 and 27.5: The transition from vendor evaluation to compute planning is implicit. A sentence about "once you select vendors, the next question is how much compute to provision" would help. (TIER 2)
...
**Clarity Issues:**
1. Section 27.2: The LLMProductSpec code example is 35+ lines, introducing many fields at once. Could benefit from a brief prose walkthrough before the code. (TIER 3)
2. Section 27.3: ConversationalAgentROI has 13 fields; students may feel overwhelmed seeing all at once. (TIER 3)
...
**Missing Analogies:**
1. Section 27.1: The readiness assessment could use an analogy. "Think of it like a preflight checklist: you would not take off with one engine untested." (TIER 2)
2. Section 27.5: Breakeven analysis could use "renting vs. buying a car" analogy for API vs. self-hosted. (TIER 2)
...
2. Section 27.3: The lab is excellent but could have a "modify the model" extension exercise. (TIER 3)
...
**Issues:**
1. Section 27.2: The `generate_stakeholder_update` function output shows "5/5 metrics on target" but the deflection metric was WARN, not PASS. This is because the health_check only counts "PASS" (not "WARN"), so 4 PASS + 1 WARN = 4/5 should print, but the output shows 5/5. The health_check returns 5 items where only deflection is WARN, so the count of PASS values should be 4, not 5. (TIER 1, factual error in output)
2. Section 27.4: InferenceRouter `select_provider` with priority "cost" returns `max(available, key=lambda p: p.weight)`. But higher weight means MORE traffic, not lower cost. The self-hosted provider has the lowest weight (0.15). This logic is inverted. (TIER 1, logic bug)
...
**Visual Inventory:**
- Section 27.1: 3 diagrams (Readiness Radar, Value-Complexity Matrix, Roadmap)
- Section 27.2: 2 diagrams (Hallucination Defense Layers, Iteration Cycle)
- Section 27.3: 2 diagrams (ROI Framework, Attribution Challenge)
- Section 27.4: 1 diagram (Build vs. Buy Decision Tree)
- Section 27.5: 2 diagrams (Cost Curves, Multi-Cloud Architecture)
...
**Errors Found:**
1. Section 27.2: Code output "5/5 metrics on target" is incorrect; deflection is WARN so it should be "4/5". (TIER 1, CERTAIN)
2. Section 27.5: "2024/2025" in the GPU selection intro is slightly outdated phrasing for a 2026 course. Should say "2024 to 2026" or simply refer to "current generation." (TIER 2)
3. Section 27.3: GPT-4o-mini pricing stated as "$3/$12 per million tokens" conflicts with the 27.5 section which states "$0.15/$0.60 per million tokens." The 27.5 values appear more current. (TIER 1, inconsistency)

**Needs Qualification:**
1. Section 27.4: "Claude 3.5 Sonnet" and "Claude 3.5 Haiku" referenced in code; should note model names change over time. Already covered by the Note callout.
...
1. Section 27.2: Mentions RAG and tool use in hallucination defense without referencing Module 19 (RAG) or Module 21 (Agents). (TIER 2)
2. Section 27.4: Mentions vector databases without referencing Module 19 where they were first introduced. (TIER 2)
3. Section 27.5: References vLLM without noting where inference serving was introduced. (TIER 3)
...
**Humor Opportunities:**
1. Section 27.1: The "Demo Trap" warning could open with a vivid scenario. (TIER 3)
2. Section 27.3: The $0.29 vs $8.50 comparison is already a natural "aha" stat.
...
**Depth Opportunities:**
1. Section 27.3: "Paper Spotlight" sidebar on the McKinsey/BCG studies quantifying enterprise AI ROI would add credibility. (TIER 3)
2. Section 27.5: Mention of inference optimization research (speculative decoding, KV cache compression) as frontier topics. (TIER 3)
...
1. Section 27.4: Agent framework table mentions LangChain, LlamaIndex, Semantic Kernel but not newer frameworks (CrewAI, AutoGen, Pydantic AI). (TIER 3)
2. Section 27.5: GPU table could include B100/B200 as upcoming tier. (TIER 3)
3. Section 27.2: Model names like "Claude 3.5 Sonnet" may not be current. The Note callout covers this. (TIER 3)
...
1. After Section 27.1: "You Could Build This: An AI readiness assessment tool for your own organization using the ReadinessAssessment class." (TIER 3)
2. After Section 27.3: "You Could Build This: A Streamlit dashboard that takes ROI model parameters and shows breakeven charts." (TIER 2)
...
1. Section 27.3: "$0.29 vs $8.50 per conversation" is a powerful number that shocks.
2. Section 27.1: RICE scoring where ticket routing dominates contract review despite lower per-unit impact.
3. Section 27.5: Breakeven at 5.3M requests/month showing most orgs should use APIs.

**Missing Aha Moments:**
1. Section 27.3: A "Year 1 vs Year 2" side-by-side for the support AI ROI would make the multi-year value click. (TIER 2)
...
2. Section 27.5: Could mention emerging GPU architectures and inference optimization as active areas. (TIER 3)
...
1. Section 27.5: A parameter sensitivity exploration (change deflection_rate and see ROI change) would be a strong interactive moment. (TIER 3)
...
**Energy Map:**
- Section 27.1: HIGH (good hook, varied examples)
- Section 27.2: HIGH (practical metrics, UX patterns)
- Section 27.3: HIGH (concrete ROI models, compelling numbers)
- Section 27.4: MEDIUM-HIGH (comparison tables, decision tree)
- Section 27.5: HIGH (GPU specs, breakeven analysis, multi-cloud)
