...
|---|--------|----------|----------|------------|---------|
| 1 | LoRA decomposition (15.1.3) | `lora-weights-raschka.png` | [`fig-15.1.3-lora-decomposition.svg`](fig-15.1.3-lora-decomposition.svg) | 195 KB | Frozen W₀ + trainable B·A; forward pass shows `h = W₀x + BAx`; explicit shape annotations |
...
| 3 | RAG pipeline (20.1.3) | `rag-pipeline-nvidia.png` | [`fig-20.1.3-rag-pipeline.svg`](fig-20.1.3-rag-pipeline.svg) | 321 KB | Two-lane: A (offline ingest) + B (online query) with numbered steps + failure-points callout |
...
| 6 | DDPM diffusion (27.1.3) | `ddpm-forward-reverse-process.png` | [`fig-27.1.3-ddpm-process.svg`](fig-27.1.3-ddpm-process.svg) | 368 KB | Forward (top, red) + reverse (bottom, green) with progressive noise illustrations + equations |
| 7 | GraphRAG (20.3.4) | `fig-20.3-graphrag-pipeline.png` | [`fig-20.3.4-graphrag-pipeline.svg`](fig-20.3.4-graphrag-pipeline.svg) | 312 KB | Phase A indexing (5 steps) + Phase B query routing (router → local OR global → LLM) |
...
SRC=KDP/diagrams/svg/fig-15.1.3-lora-decomposition.svg
DST=part-4-training-adapting/module-15-fine-tuning-peft-lora-qlora/images/lora-weights-raschka.svg
cp "$SRC" "$DST"
...
|---|--------|-------------------|
| 1 | LoRA | `part-4-training-adapting/module-15-fine-tuning-peft-lora-qlora/images/` |
| 2 | BPE | `part-1-foundations/module-02-tokenization-subword-models/images/` |
| 3 | RAG | `part-5-retrieval-conversation/module-20-rag/images/` |
| 4 | RLHF PPO | `part-4-training-adapting/module-17-alignment-rlhf-dpo/images/` |
| 5 | RLHF three-stage | `part-4-training-adapting/module-17-alignment-rlhf-dpo/images/` |
| 6 | DDPM | `part-7-multimodal-applications/module-27-multimodal/images/` |
| 7 | GraphRAG | `part-5-retrieval-conversation/module-20-rag/images/` |

After promotion, also remove the third-party attribution from each `figcaption` (since the diagrams are now original work):
```html
<!-- BEFORE -->
<div class="diagram-caption"><strong>Figure 20.1.3</strong>: ... Source: NVIDIA, 2023. <a href="...">RAG 101...</a></div>

<!-- AFTER -->
<div class="diagram-caption"><strong>Figure 20.1.3</strong>: ...</div>
```
