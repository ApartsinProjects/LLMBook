...
**Date:** 2026-03-26
**Files reviewed:** index.html, section-23.1.html, section-23.2.html, section-23.3.html, section-23.4.html
...
**Coverage Gap:**
- TIER 1: Learning objective "Evaluate agent performance using task completion, efficiency, and safety metrics" has no dedicated coverage. Section 23.1 discusses failure modes but does not cover benchmarks (SWE-bench, GAIA, AgentBench, etc.) or evaluation frameworks.
...
**Issues:**
- TIER 2: Section 23.3 LATS: the UCT formula is used but the intuition behind exploration vs. exploitation tradeoff could use one more sentence of explanation
- TIER 3: Section 23.2 MCP: the protocol details (JSON-RPC transport) are mentioned but not explained; a sentence on why JSON-RPC was chosen would help
- TIER 3: Section 23.4 mentions Firecracker microVMs vs. gVisor but does not explain the security tradeoff between them
...
**Issues:**
- TIER 2: Section 23.1 jumps from ReAct (Section 3) to cognitive architectures (Section 4) without a bridge sentence
- TIER 2: Section 23.3 transitions from LATS (Section 4) to LLM Compiler (Section 5) abruptly; these are quite different approaches and need a connecting sentence
...
**Issues:**
- TIER 2: Section 23.1 introduces "cognitive architecture" (Section 4) without defining it first; a student would wonder what this term means
- TIER 2: Section 23.2 A2A protocol section (Section 7) is brief; students may wonder how A2A differs from simple REST APIs between services
- TIER 3: Section 23.3 uses "UCT (Upper Confidence Bound for Trees)" without explaining what "upper confidence bound" means intuitively
...
**Issues:**
- TIER 3: Section 23.1 Section 4 (cognitive architectures + state machines) introduces AgentState, AgentContext, StatefulAgent, and state machine transitions in one subsection; could benefit from a brief "what just happened" summary after the code block
- TIER 3: Section 23.2 Section 6 (MCP) + Section 7 (A2A) are dense with new protocols; a comparison callout would help
...
**Issues:**
- TIER 2: Section 23.1 introduces the perception-reasoning-action loop but the only analogy is implicit; adding an explicit analogy (e.g., comparing to a detective investigating a case) would strengthen retention
- TIER 2: Section 23.2 MCP section uses the USB analogy but it appears only once; reinforcing it would help
- TIER 3: Section 23.3 LATS could benefit from a game-tree analogy (chess) to ground the tree search concept
...
- TIER 3: Section 23.3 lab is good but could add a "stretch goal" for students who finish early
...
**Issues:**
- TIER 2: Section 23.2 Anthropic code example (line 289) uses model name "claude-sonnet-4-20250514" which may need updating as models evolve; consider using a comment noting the model name should be updated
- TIER 3: Section 23.1 ReAct agent uses text parsing (ACTION: tool_name(args)); the note correctly says production would use native function calling, but a brief "here is what the native version looks like" pointer to 21.2 would strengthen the bridge
- TIER 3: Section 23.4 CodeSecurityChecker blocks `open` builtin, but the data analysis code uses `pd.read_csv` which internally calls open; a note about this nuance would be educational
...
**Issues:**
- TIER 2: Section 23.2 MCP/A2A section: no diagram comparing MCP (vertical) vs A2A (horizontal) connections; this concept cries out for a visual
- TIER 3: Section 23.1 four agentic patterns diagram is good but could label the connecting relationships between patterns
...
**Issues:**
- TIER 1: Section 23.1 "Agents vs. Chains vs. Workflows" could lead students to think these are strict categories when in practice the boundaries are blurry. Add a note: "In practice, many systems combine elements of all three."
- TIER 2: Section 23.4 "static code filtering alone is never sufficient" is stated correctly but students may still over-rely on it. The callout is good but could be more prominent.
...
**Issues:**
- TIER 2: Section 23.2 states Anthropic recommends "under 20" tools; this should be qualified as a performance recommendation, not a hard limit
- TIER 2: Section 23.3 attributes plan-and-execute to "LangGraph team and inspired by the BabyAGI project"; while accurate, noting that plan-and-execute as a general AI pattern predates both would be more precise
- TIER 3: Section 23.2 says Google proposed A2A "in 2025"; confirm this is the correct year (it is: April 2025)
...
**Issues:**
- TIER 3: Section 23.1 uses "perception-reasoning-action loop" and later "perception-reasoning-action cycle"; standardize to one term
- TIER 3: Section 23.3 refers to "plan-and-execute" with hyphens consistently, which is good
...
References to Module 19 (RAG), Module 20 (conversational AI), and Module 22 (multi-agent) exist. Tool use in 21.1 correctly points to "Section 23.2 covers this in depth."

**Issues:**
- TIER 2: Section 23.1 mentions RAG (Module 19) for semantic memory but does not link back to Module 10 for prompt engineering techniques used in agent prompts
- TIER 2: Section 23.4 SWE agents section does not reference Module 22 (multi-agent), but SWE agents like Devin use multi-agent architectures internally
- TIER 3: Section 23.2 native tool use training could reference Module 08 or 09 (API training, RLHF)
...
- TIER 2: Section 23.1 could open with a more vivid hook; the current opening is solid but not grabbing
- TIER 3: Section 23.3 is the most "textbook-like" section and could use a surprising fact or counterintuitive example early on
...
**Issues:**
- TIER 3: Section 23.3 has numbered sections ("1. Why Planning Matters") through "7. Combining Patterns" plus a Lab; this is a lot of sections and could benefit from grouping into two H2-level themes
...
**Issues:**
- TIER 3: Section 23.3 mentions "Monte Carlo Tree Search" without a brief explanation of what Monte Carlo methods are; most software engineers would know this, but a parenthetical would be safe
...
- TIER 2: Section 23.1 title "Foundations of AI Agents" is functional but not compelling; consider "What Makes an AI Agent Think?" or similar
- TIER 3: Section 23.3 subtitle is long; consider trimming
...
**Issues:**
- TIER 2: Section 23.2 could benefit from a "before/after tool use" example showing the same query with and without tools, demonstrating why tools are necessary
- TIER 3: Section 23.4 self-debugging section could show a concrete before/after of error correction
...
- TIER 2: Section 23.1 opening could start with a more vivid scenario (e.g., "Imagine asking an AI to plan your vacation, book the flights, and write the packing list")
...
**Issues:**
- TIER 3: Section 23.1 could suggest a "try modifying the max_steps parameter and observe how it affects agent behavior" demo
- TIER 3: Section 23.4 could suggest uploading a personal CSV to the data analysis agent
...
**Issues:**
- TIER 3: Section 23.3 Big Picture: "Without planning, an agent reacts to each user message in isolation, choosing one tool at a time" could be simplified to "Without planning, an agent picks one tool at a time and hopes for the best"
...
**Issues:**
- TIER 3: Section 23.1 opening paragraph has three sentences of similar length and structure; varying the rhythm would improve readability
...
**Issues:**
- TIER 3: Section 23.1 Section 5 (Agent Memory Systems) could use a comparison table for the three memory types instead of sequential subsections
- TIER 3: Section 23.2 Section 9 (Native Tool Use Training) is two paragraphs with no visual break; a brief list or callout would improve scannability
...
**Issues:**
- TIER 3: Section 23.3 middle sections (LATS code + LLM Compiler code) are back-to-back dense code blocks; a brief "take a breath" summary between them would help
- TIER 3: Section 23.4 is the longest section and comes last; fatigue may set in. The lab at the end is engaging, which helps.
